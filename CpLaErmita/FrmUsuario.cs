// ============================================================
// FrmUsuario.cs
// Formulario WinForms independiente del módulo Usuarios.
...
namespace CpLaErmita
{
    public partial class FrmUsuario : Form
    {
        ...
        private void CargarDatosIniciales()
        {
            try
            {
                CargarCombo(cmbRol, "Roles", "nombre");
            }
            catch (Exception ex)
            {
        ...
        private void Listar()
        {
            try
            {
                DataTable lista = ProfesionalCln.Listar("Usuarios", txtParametro.Text);
                dgvLista.DataSource = lista;

                if (dgvLista.Columns.Contains("id")) dgvLista.Columns["id"].Visible = false; if (dgvLista.Columns.Contains("idRol")) dgvLista.Columns["idRol"].Visible = false;
                if (dgvLista.Columns.Contains("rol")) dgvLista.Columns["rol"].HeaderText = "Rol"; if (dgvLista.Columns.Contains("nombre")) dgvLista.Columns["nombre"].HeaderText = "Nombre"; if (dgvLista.Columns.Contains("usuario")) dgvLista.Columns["usuario"].HeaderText = "Usuario"; if (dgvLista.Columns.Contains("email")) dgvLista.Columns["email"].HeaderText = "Email"; if (dgvLista.Columns.Contains("telefono")) dgvLista.Columns["telefono"].HeaderText = "Teléfono"; if (dgvLista.Columns.Contains("estado")) dgvLista.Columns["estado"].HeaderText = "Estado";
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            SeleccionarCombo(cmbRol, ValorCeldaObjeto("idRol")); txtNombre.Text = ValorCelda("nombre"); txtUsuario.Text = ValorCelda("usuario"); txtContrasena.Text = ValorCelda("contrasena"); txtEmail.Text = ValorCelda("email"); txtTelefono.Text = ValorCelda("telefono");
            cmbRol.Focus();
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                valores["idRol"] = ObtenerValorCombo(cmbRol); valores["nombre"] = txtNombre.Text.Trim(); valores["usuario"] = txtUsuario.Text.Trim(); valores["contrasena"] = txtContrasena.Text.Trim(); valores["email"] = txtEmail.Text.Trim(); valores["telefono"] = txtTelefono.Text.Trim();

                ProfesionalCln.Guardar("Usuarios", esNuevo ? 0 : idActual, valores);
                Listar();
        ...
        private void CambiarEstado(short estado, string accion)
        {
            ...
            try
            {
                ProfesionalCln.CambiarEstadoLogico("Usuarios", id, estado);
                Listar();
            }
            catch (Exception ex)
            {
        ...
        {
            if (cmbRol.Items.Count > 0) cmbRol.SelectedIndex = 0; txtNombre.Clear(); txtUsuario.Clear(); txtContrasena.Clear(); txtEmail.Clear(); txtTelefono.Clear();
            ResetearErrores();
        }
        ...
        private bool Validar()
        {
            bool ok = true;
            ResetearErrores();
            if (cmbRol.SelectedValue == null) { erpPrincipal.SetError(cmbRol, "Seleccione Rol."); ok = false; } if (string.IsNullOrWhiteSpace(txtNombre.Text)) { erpPrincipal.SetError(txtNombre, "El campo Nombre es obligatorio."); ok = false; } if (string.IsNullOrWhiteSpace(txtUsuario.Text)) { erpPrincipal.SetError(txtUsuario, "El campo Usuario es obligatorio."); ok = false; }

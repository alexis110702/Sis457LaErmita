// ============================================================
// FrmProveedor.cs
// Formulario WinForms independiente del módulo Proveedores.
...
namespace CpLaErmita
{
    public partial class FrmProveedor : Form
    {
        ...
        private void Listar()
        {
            try
            {
                DataTable lista = ProfesionalCln.Listar("Proveedores", txtParametro.Text);
                dgvLista.DataSource = lista;

                if (dgvLista.Columns.Contains("id")) dgvLista.Columns["id"].Visible = false;
                if (dgvLista.Columns.Contains("nombre")) dgvLista.Columns["nombre"].HeaderText = "Nombre"; if (dgvLista.Columns.Contains("nit")) dgvLista.Columns["nit"].HeaderText = "NIT"; if (dgvLista.Columns.Contains("telefono")) dgvLista.Columns["telefono"].HeaderText = "Teléfono"; if (dgvLista.Columns.Contains("email")) dgvLista.Columns["email"].HeaderText = "Email"; if (dgvLista.Columns.Contains("direccion")) dgvLista.Columns["direccion"].HeaderText = "Dirección"; if (dgvLista.Columns.Contains("contacto")) dgvLista.Columns["contacto"].HeaderText = "Contacto"; if (dgvLista.Columns.Contains("estado")) dgvLista.Columns["estado"].HeaderText = "Estado";
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            txtNombre.Text = ValorCelda("nombre"); txtNit.Text = ValorCelda("nit"); txtTelefono.Text = ValorCelda("telefono"); txtEmail.Text = ValorCelda("email"); txtDireccion.Text = ValorCelda("direccion"); txtContacto.Text = ValorCelda("contacto");
            txtNombre.Focus();
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                valores["nombre"] = txtNombre.Text.Trim(); valores["nit"] = txtNit.Text.Trim(); valores["telefono"] = txtTelefono.Text.Trim(); valores["email"] = txtEmail.Text.Trim(); valores["direccion"] = txtDireccion.Text.Trim(); valores["contacto"] = txtContacto.Text.Trim();

                ProfesionalCln.Guardar("Proveedores", esNuevo ? 0 : idActual, valores);
                Listar();
        ...
        private void CambiarEstado(short estado, string accion)
        {
            ...
            try
            {
                ProfesionalCln.CambiarEstadoLogico("Proveedores", id, estado);
                Listar();
            }
            catch (Exception ex)
            {
        ...
        {
            txtNombre.Clear(); txtNit.Clear(); txtTelefono.Clear(); txtEmail.Clear(); txtDireccion.Clear(); txtContacto.Clear();
            ResetearErrores();
        }
